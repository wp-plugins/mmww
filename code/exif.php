...
function mmww_get_exif_metadata ($file) {
	
	$meta = array();
	
	// fetch additional info from exif if available
	if ( ! is_callable( 'exif_read_data' ) ) {
		/* no exif handler present */
		$meta['warning'] = __('Your PHP installation cannot read image metadata; no exif_read_data method available', 'mmww');
	}
	else {
		...
		if (	(! empty( $exif['GPSLongitudeRef'] )) &&
			...
			$meta['longitude'] = mmww_getGPS( $exif['GPSLongitudeRef'], $exif['GPSLongitude'] );
			$meta['latitude'] =  mmww_getGPS( $exif['GPSLatitudeRef'], $exif['GPSLatitude'] );	
		}
